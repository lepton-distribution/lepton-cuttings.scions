...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
   ...
   AUDIO_OUT_CHANNEL_1_EP,                         /* bEndpointAddress 1 out endpoint*/
   ...
   AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
   USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
   AUDIO_IN_CHANNEL_1_EP,                          /* bEndpointAddress 1 IN endpoint*/
...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
   ...
   /* USB Speaker Class-specific AC Interface Descriptor */
   AUDIO_INTERFACE_DESC_SIZE+3,            /* bLength */
   AUDIO_INTERFACE_DESCRIPTOR_TYPE,      /* bDescriptorType */
   AUDIO_CONTROL_HEADER,                 /* bDescriptorSubtype */
   0x00,          /* 1.00 */             /* bcdADC */
   0x01,
   0x84,//0x46,                          /* wTotalLength = 70*/
   0x00,
   0x04,//0x02,                          /* bInCollection */
   0x01,//0x01,                          /* baInterfaceNr 1 */
   0x02,                                 /* baInterfaceNr 2*/
   0x03,//0x01,                          /* baInterfaceNr 3 */
   ...
   0x00,                                 /* bAssocTerminal */
   0x02,                                 /* bNrChannels */
   0x03,                                 /* wChannelConfig 0x0000  Mono Left*/
   0x00,
   0x00,                                 /* iChannelNames */
   0x00,                                 /* iTerminal */
   /* 12 byte*/
   ...
   0x00,                                 /* bAssocTerminal */
   0x02,                                 /* bNrChannels */
   0x03,                                 /* wChannelConfig 0x0000  Mono Right */
   0x00,
   0x00,                                 /* iChannelNames */
   0x00,                                 /* iTerminal */
   /* 12 byte*/
   ...
   AUDIO_OUT_CHANNEL_1_EP,               /* bEndpointAddress 1 out endpoint*/
   USBD_EP_TYPE_ISOC,                    /* bmAttributes : isochronous asynchronous*/
   AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*1(Mono)*2(HalfWord)) */
   0x01,                                 /* bInterval */
   0x00,                                 /* bRefresh */
   0x00,                                 /* bSynchAddress */
   /* 09 byte*/
   ...
   AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
   USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
   AUDIO_OUT_CHANNEL_2_EP,               /* bEndpointAddress 1 out endpoint*/
   USBD_EP_TYPE_ISOC,                    /* bmAttributes : isochronous asynchronous*/
   AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*1(Mono)*2(HalfWord)) */
   0x01,                                 /* bInterval */
   0x00,                                 /* bRefresh */
   ...
   AUDIO_IN_CHANNEL_1_EP,               /* bEndpointAddress 1 IN endpoint*/
   USBD_EP_TYPE_ISOCASYNC,               /* bmAttributes : isochronous asynchronous*/ //USB_ENDPOINT_TYPE_ISOCHRONOUS,        /* 01: Asynchronous, 10: Adaptive, 11: synchronous. bmAttributes */
   SET_AUDIO_IN_PACKET_SZ_MAX(AUDIO_IN_PACKET_SZ_MAX),    /* wMaxPacketSize in Bytes (Freq(Samples)*1(Mono)*2(HalfWord)) */
   0x01,                                 /* bInterval */
   0x00,                                 /* bRefresh */
   0x00,                                 /* bSynchAddress */
   /* 09 byte*/
   ...
   /* Interface 4 channel 2 Input*/ 
   
   /* Interface 4, Alternate Setting 0, Audio Streaming - Zero Bandwith */
   9,                                    /* bLength */
   USB_DESC_TYPE_INTERFACE,              /* bDescriptorType */
   ...
   /* Interface 4, Alternate Setting 1, Audio Streaming - Operational */
   9,    /* bLength */
   0x04,                                 /* bDescriptorType */
   0x04,                                 /* bInterfaceNumber */
   0x01,                                 /* bAlternateSetting */
   ...
   AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
   USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
   AUDIO_IN_CHANNEL_2_EP,               /* bEndpointAddress 1 IN endpoint*/
   USBD_EP_TYPE_ISOCASYNC,               /* bmAttributes : isochronous asynchronous*/ //USB_ENDPOINT_TYPE_ISOCHRONOUS,        /* 01: Asynchronous, 10: Adaptive, 11: synchronous. bmAttributes */
   SET_AUDIO_IN_PACKET_SZ_MAX(AUDIO_IN_PACKET_SZ_MAX),    /* wMaxPacketSize in Bytes (Freq(Samples)*1(Mono)*2(HalfWord)) */
   0x01,                                 /* bInterval */
   0x00,                                 /* bRefresh */
...
#pragma data_alignment=4 
static int16_t IsocAudioInBuffer[USB_AUDIO_CHANNEL_MAX][AUDIO_IN_PACKET_SZ_MAX]={0};
...
#pragma data_alignment=4 
static audio_ring_buffer_info_t audio_ring_buffer_channel[USB_AUDIO_CHANNEL_MAX];

// sinus signal test
#pragma data_alignment=4 
int16_t Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_MAX][AUDIO_IN_PACKET_SZ_MAX]={0};
...
//
static void convert_16_to_24_bit(usb_24bits_format_byte_t *p_24bits_buffer, uint16_t *p_16bits_buffer, int sample_nb){
  int i=0;
  //
...
static void init_sinus_16_bit_48KHz(int16_t *p_16bits_buffer){
  int i;
   float PI=3.14159265;
...
static void init_sinus_24_bit_48KHz(usb_24bits_format_byte_t *p_24bits_buffer){
  int i;
  float PI=3.14159265;
  int32_t Amax= (0x7FFF>>2); //8388607/10;
...
//
static int audio_channel_init(void){
  ...
  audio_ring_buffer_channel[USB_AUDIO_CHANNEL_INPUT_2].p_buffer = USBD_malloc(USB_AUDIO_CHANNEL_INPUT_SZ);
  audio_ring_buffer_channel[USB_AUDIO_CHANNEL_INPUT_2].sz=USB_AUDIO_CHANNEL_INPUT_SZ;
  audio_ring_buffer_channel[USB_AUDIO_CHANNEL_INPUT_2].r=0;
  audio_ring_buffer_channel[USB_AUDIO_CHANNEL_INPUT_2].w=0;
  
  return 0;
}

//
static int audio_channel_ring_buffer_write(int audio_channel_nb,uint8_t *buffer,int size){
  ...
  if(size>free_sz)
   ...
   //to do
   //audio_ring_buffer_channel[audio_channel_nb].w=w;
   
   //
   return size;
}

static int audio_channel_ring_buffer_read(int audio_channel_nb,uint8_t *buffer,int size){
   int16_t r= audio_ring_buffer_channel[audio_channel_nb].r;
   int16_t w= audio_ring_buffer_channel[audio_channel_nb].w;
   int16_t sz = audio_ring_buffer_channel[audio_channel_nb].sz;
   ...
   //to do
   //audio_ring_buffer_channel[audio_channel_nb].r=r;
   //
   return size;
}
                               ...
                               uint8_t cfgidx)
{
  ...
  /* Open EP OUT */
#ifdef AUDIO_OUT_CHANNEL_1_EP
  USBD_LL_OpenEP(pdev,
                 AUDIO_OUT_CHANNEL_1_EP,
                 USBD_EP_TYPE_ISOC,
                 AUDIO_OUT_PACKET);
#endif

#ifdef AUDIO_OUT_CHANNEL_2_EP
  USBD_LL_OpenEP(pdev,
                 AUDIO_OUT_CHANNEL_2_EP,
                 USBD_EP_TYPE_ISOC,
                 AUDIO_OUT_PACKET);
#endif
  
  /* Open EP IN */
#ifdef AUDIO_IN_CHANNEL_1_EP
  USBD_LL_OpenEP(pdev,
                 AUDIO_IN_CHANNEL_1_EP,
                 USBD_EP_TYPE_ISOC,
                 AUDIO_IN_PACKET_SZ_MAX);
#endif

#ifdef AUDIO_IN_CHANNEL_2_EP
  USBD_LL_OpenEP(pdev,
                 AUDIO_IN_CHANNEL_2_EP,
                 USBD_EP_TYPE_ISOC,
                 AUDIO_IN_PACKET_SZ_MAX);
#endif
  ...
  else
  {
    ...
    init_sinus_16_bit_48KHz_ex(Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_INPUT_1],1000);
    init_sinus_16_bit_48KHz_ex(Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_INPUT_2],2000);
    //
    ...
    //
#ifdef AUDIO_IN_CHANNEL_1_EP
    USBD_LL_FlushEP(pdev, AUDIO_IN_CHANNEL_1_EP);
#endif
#ifdef AUDIO_IN_CHANNEL_2_EP
    USBD_LL_FlushEP(pdev, AUDIO_IN_CHANNEL_2_EP);
#endif
    //

#if defined(AUDIO_IN_CHANNEL_1_EP) && defined(I2S_24BIT)
    USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_1_EP, (uint8_t*)IsocInBuffDummy24bit, 16*2*HALF_WORD_BYTES);
#endif
    
#if defined(AUDIO_IN_CHANNEL_2_EP) && defined(I2S_24BIT)
    USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_2_EP, (uint8_t*)IsocInBuffDummy24bit, 16*2*HALF_WORD_BYTES);
#endif
    
#if defined(AUDIO_IN_CHANNEL_1_EP)
    USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_1_EP, (uint8_t*)IsocInBuffDummy16bit, AUDIO_IN_PACKET_SZ);
#endif
    
#if defined(AUDIO_IN_CHANNEL_2_EP)
    USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_2_EP, (uint8_t*)Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_INPUT_2], AUDIO_IN_PACKET_SZ);
#endif 
  }
  return USBD_OK;
}
       ...
       switch (req->bRequest)
       {
          case USB_REQ_GET_DESCRIPTOR:      
            if( (req->wValue >> 8) == AUDIO_DESCRIPTOR_TYPE)
            {
              ...
              len = MIN(USB_AUDIO_DESC_SIZ+3 , req->wLength);
          ...
          case USB_REQ_SET_INTERFACE :
            if ((uint8_t)(req->wValue) <= USBD_MAX_NUM_INTERFACES)
            {
              ...
              //req->wValue = alt setting number
               ...
               if(req->wIndex=3 && req->wValue==1){
                  /* Prepare In endpoint to transmit 1st packet */
                  //USBD_LL_FlushEP(pdev, AUDIO_IN_CHANNEL_1_EP);
                  //USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_1_EP, (uint8_t*)IsocInBuffDummy16bit, AUDIO_IN_PACKET_SZ);
               }
               
               if(req->wIndex=4 && req->wValue==1){
                  /* Prepare In endpoint to transmit 1st packet */
                  //USBD_LL_FlushEP(pdev, AUDIO_IN_CHANNEL_2_EP);
                  //USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_2_EP, (uint8_t*)Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_INPUT_2], AUDIO_IN_PACKET_SZ);
               }
                              ...
                              uint8_t epnum)
{
   ...
   if (epnum == (AUDIO_IN_CHANNEL_1_EP&0x7F))
   {
    USBD_LL_FlushEP(pdev, USB_AUDIO_CHANNEL_INPUT_1);
   #ifdef I2S_24BIT
       USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_1_EP, (uint8_t*)IsocInBuffDummy24bit, 16*2*HALF_WORD_BYTES);
   #else
       ...
       //
       USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_1_EP, (uint8_t*)IsocAudioInBuffer[USB_AUDIO_CHANNEL_INPUT_1], AUDIO_IN_PACKET_SZ);
...
#ifdef AUDIO_IN_CHANNEL_2_EP
   /* Prepare next buffer to be sent: dummy data */
   if (epnum == (AUDIO_IN_CHANNEL_2_EP&0x7F))
   {
    USBD_LL_FlushEP(pdev, USB_AUDIO_CHANNEL_INPUT_2);
   #ifdef I2S_24BIT
       USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_2_EP, (uint8_t*)IsocInBuffDummy24bit, 16*2*HALF_WORD_BYTES);
   #else
       audio_channel_ring_buffer_write(USB_AUDIO_CHANNEL_INPUT_2,(uint8_t*)Sinus_48KHz_16bits[USB_AUDIO_CHANNEL_INPUT_2],AUDIO_IN_PACKET_SZ);
       audio_channel_ring_buffer_read(USB_AUDIO_CHANNEL_INPUT_2,(uint8_t*)IsocAudioInBuffer[USB_AUDIO_CHANNEL_INPUT_2],AUDIO_IN_PACKET_SZ);
       //memset((uint8_t*)IsocAudioInBuffer[USB_AUDIO_CHANNEL_INPUT_2],0,AUDIO_IN_PACKET_SZ);
       USBD_LL_Transmit(pdev, AUDIO_IN_CHANNEL_2_EP, (uint8_t*)IsocAudioInBuffer[USB_AUDIO_CHANNEL_INPUT_2], AUDIO_IN_PACKET_SZ);
    #endif 
   }
#endif
                              ...
                              uint8_t epnum)
{
  ...
  if (epnum == AUDIO_OUT_CHANNEL_1_EP)
  {
